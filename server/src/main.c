...
int handle_command_4(command_4_request* req, unsigned char* data)
{
  ...
  resp.command = PSVDMAC5_COMMAND_HMACSHA1;
  resp.proxy_err = 0;
...
int handle_command_5(command_5_request* req, unsigned char* data)
{
  ...
  resp.command = PSVDMAC5_COMMAND_AESCMAC;
  resp.proxy_err = 0;
...
int handle_command_4_base(int command)
{
  ...
  //receive main data
  int header_size = sizeof(command_4_request) - sizeof(int);
  char* header_dest = ((char*)&recvBuffer) + sizeof(int);
  if(recv_data(header_dest, header_size) < 0)
    return -1;

  //check arguments
  if(recvBuffer.size == 0 || recvBuffer.mask_enable != 1 || recvBuffer.command_bit != 0)
  {
    psvDebugScreenPrintf("psvdmac5: invalid args in command 4\n");
    return -1;
  }

  //allocate data buffer
  int data_size = recvBuffer.size + 0x20;
  unsigned char* data_dest = 0;
  SceUID data_uid = -1;
  ...
  //handle command
  if(handle_command_4(&recvBuffer, data_dest) < 0)
  {
    deallocate_buffer(data_uid);
    psvDebugScreenPrintf("psvdmac5: failed to handle command 4\n");
    return -1;
  }
...
int handle_command_5_base(int command)
{
  ...
  //receive main data
  int header_size = sizeof(command_5_request) - sizeof(int);
  char* header_dest = ((char*)&recvBuffer) + sizeof(int);
  if(recv_data(header_dest, header_size) < 0)
    return -1;

  //check arguments
  if(recvBuffer.size == 0 || recvBuffer.key_size == 0 || recvBuffer.mask_enable != 1 || recvBuffer.command_bit != 0)
  {
    psvDebugScreenPrintf("psvdmac5: invalid args in command 5\n");
    return -1;
  }
  ...
  //handle command
  if(handle_command_5(&recvBuffer, data_dest) < 0)
  {
    deallocate_buffer(data_uid);
    psvDebugScreenPrintf("psvdmac5: failed to handle command 5\n");
    return -1;
  }
